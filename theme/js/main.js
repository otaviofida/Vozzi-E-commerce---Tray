(($) => {
  ...
  window.theme = {
    ...
    scrollHidesMenu: function () {
      ...
      if (
        position > this.settings.lastScrollPosition &&
    ...
    getShippingRates: function (url) {
      $.ajax({
        ...
        success: function (response) {
          ...
          if (
            textError.includes("estimar o valor do frete. Clique em Continuar")
          ) {
            let message =
              "N&atilde;o foi poss&iacute;vel obter os pre&ccedil;os e prazos de entrega.";
            resultBox
              .removeClass("loading")
              .addClass("loaded")
              .html(`<p class="error-block">${message}</p>`);
    ...
    goToProductReviews: function () {
      const ratingStars = $(
        ".pageProduct .pageProduct-nameAndInformation .product-rating",
      );
      const internal = this;
    ...
    reviewsOnProductPage: function () {
      let commentsBlock = $(
        `<div class="tabs-reviews">${window.commentsBlock}</div>`,
      );
      const buttonReview =
    ...
    buyTogetherOnProductPage: function () {
      ...
      const qtd = $(
        ".compreJunto .precoCompreJunto .unidades_preco .unidades_valor",
      );
      ...
      image.each(function (index) {
        ...
        $(this)
          .addClass("buyTogether-img lazyload")
          .attr("src", "")
          .attr("data-src", bigImgUrl);
        ...
        if (link !== "") {
          ...
          $(this)
            .parents("span")
            .after(
              `<a class="buyTogether-nameProduct" href="${link}">${name}</a>`,
            );
        } else {
          $(this)
            .parents("span")
            .after(`<p class="buyTogether-nameProduct">${name}</p>`);
        }

        if (listQtd[index] == 1) {
          $(this).after(
            `<p class="buyTogether-text">${listQtd[index]} unidade</p>`,
          );
        } else {
          $(this).after(
            `<p class="buyTogether-text">${listQtd[index]} unidades</p>`,
          );
        }
      });
    },
    ...
    insertBreadcrumbNavigationInPage: function (
      local = "",
      customName = false,
    ) {
      let items;
    ...
    validateFormFieldsToSendCustomerReview: function () {
      ...
      const buttonToSendReview = $(
        ".page-depoimentos .container3 #depoimento .btn_submit",
      );
    ...
    validateFormFieldsToSendContactEmail: function () {
      const formToSendContact = $(
        ".page-contact .container2 .formulario-contato",
      );
      const buttonToSendContact = $(
        ".page-contact .container2 .formulario-contato .btn_submit",
      );
    ...
    organizeNewsletterRegistrationPage: function () {
      if ($(".page-newsletter .formulario-newsletter").length) {
        ...
        $(".page-newsletter .default-content").addClass(
    ...
    organizePagesTray: function () {
      ...
      const titleReviewPage = $(
        ".page-depoimentos .container #comentario_cliente",
      );
      const buttonAdvancedSearch = $(
        '.page-search #Vitrine input[type="image"]',
      );

      login.attr("placeholder", "Digite seu e-mail*");
      buttonReviewPage
        .html("Enviar Depoimento")
        .addClass("button2 review-button");
